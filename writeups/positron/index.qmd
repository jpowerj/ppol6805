...
For both MacOS and Windows, you can download the latest version of Positron from [its homepage](https://positron.posit.co/). Once you have downloaded and installed it follow these instructions to **connect to your JupyterHub space through Positron**:
...
Now, from among the three options shown, select the **Connect Current Window to Host...** option. The interface will then prompt you with the text `"Enter [user@]hostname[:port]"`, with a blank text box underneath this prompt. Please enter the following address, where you should replace the `YOUR_NETID` portion with your actual Georgetown-assigned **NetID**:
